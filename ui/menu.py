...
class MainMenuView(cfg.PantallaBase):
    ...
    def open_load_hours_ui(self):
        ...
        # Instancia la UI pasándole la ventana secundaria y el ID de sesión del usuario
        HorasExtensionUI(ventana_registro, self.usuario.id_user, callback_actualizar=self.load_total_hours)

    def open_summary_ui(self):
        ventana_resumen = tk.Toplevel(self)
        ResumenUI(ventana_resumen, self.usuario)
